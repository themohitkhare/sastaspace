...
class AccessoriesAnalyzerTest < ActiveSupport::TestCase
  ...
  test "parse_analysis_response extracts JSON from multiline content" do
    content = "Here is some text before\n{\"item_type\":\"accessories\",\"colors\":[\"red\",\"blue\"],\"confidence\":0.9}\nAnd text after"
  ...
  test "accessories_analysis_prompt includes item name" do
    @item.update(name: "Leather Bag")
    prompt = @analyzer.send(:accessories_analysis_prompt)

    assert_includes prompt, "Leather Bag"
  end

  test "accessories_analysis_prompt handles nil category" do
  ...
  test "accessories_analysis_prompt handles nil brand" do
  ...
  test "perform_analysis creates user message and calls chat.ask" do
    ...
    mock_assistant_message.stubs(:content).returns('{"item_type":"accessories","confidence":0.8}')
  ...
  test "accessories_analysis_prompt includes all required fields" do
    prompt = @analyzer.send(:accessories_analysis_prompt)
