...
class ComfyUiService
  ...
  class << self
    ...
    # @param inventory_item_id [Integer] Optional inventory item ID for resumption
    # @return [Hash] Result containing extracted image data or error
    def extract_stock_photo(original_image_blob:, extraction_prompt:, timeout: 300, inventory_item_id: nil)
      Rails.logger.info "Starting ComfyUI stock photo extraction (timeout: #{timeout}s)"
      ...
      # Smart resumption logic: Check if we already have a running job for this item
      if inventory_item_id.present?
        inventory_item = InventoryItem.find_by(id: inventory_item_id)
        if inventory_item&.comfyui_prompt_id.present?
          existing_job_id = inventory_item.comfyui_prompt_id
          Rails.logger.info "Checking status of existing ComfyUI job #{existing_job_id} for item #{inventory_item.id}"

          # Check if this job is still valid in ComfyUI history or queue
          # We'll use poll_for_completion with 1 attempt to check history quickly
          begin
            # Quick check history
            ...
            response = http.get("/history/#{existing_job_id}")
            if response.code == "200" && JSON.parse(response.body)[existing_job_id].present?
              Rails.logger.info "Found existing completed job in history: #{existing_job_id}"
              job_id = existing_job_id
            else
              # Check queue if not in history
              response = http.get("/queue")
              if response.code == "200"
                queue_data = JSON.parse(response.body)
                # Queue format: { "queue_running": [...], "queue_pending": [...] }
                # Each item is [number, prompt_id, ...]
                is_queued = false

                [ "queue_running", "queue_pending" ].each do |queue_type|
                  if queue_data[queue_type].is_a?(Array)
                    queue_data[queue_type].each do |item|
                      # item[1] is the prompt_id (job_id)
                      if item.is_a?(Array) && item[1] == existing_job_id
                        is_queued = true
                        break
                      end
                    end
                ...
                if is_queued
                  Rails.logger.info "Found existing job in queue: #{existing_job_id}"
                  job_id = existing_job_id
                end
              end
            end
          rescue StandardError => e
            Rails.logger.warn "Failed to check existing job status: #{e.message}"
            # Continue to submit new job if check fails
          end
        end
      end
      ...
      unless job_id
        ...
        # Save job ID to inventory item for future resumption
        if inventory_item
          Rails.logger.info "Saving ComfyUI job ID #{job_id} to inventory item #{inventory_item.id}"
          inventory_item.update_columns(comfyui_prompt_id: job_id, comfyui_status: "pending")
        end
      else
        Rails.logger.info "Resuming wait for existing job #{job_id}"
      end

      # Try WebSocket first for real-time completion, fallback to polling
      begin
        # If we are resuming, we might have missed the WebSocket connection window
        # So we should check history immediately inside the WebSocket waiter or rely on polling
        # Since we reuse the logic, let's try WebSocket (it might still catch 'executing' if running)
        # But if it was already done, WebSocket won't help unless we modify it to check history.

        # For resumption of *completed* jobs, we should just fetch results.
        # But wait_for_completion_via_websocket logic assumes we are waiting for events.
        # So if job_id was reused (resumed), let's check if it's already done first.

        if inventory_item # Only do this check if we might have resumed
           begin
             # Quick check if already done (avoid WS wait if done)
             result = fetch_job_results_after_completion(job_id)
             if result
               Rails.logger.info "Job #{job_id} was already completed, returning results immediately"
               return result
             end
           rescue
             # Not done or not found in history, continue to wait
           end
        end

        result = wait_for_completion_via_websocket(job_id, client_id: client_id, timeout: timeout)
        ...
        # Fallback to polling
        result = poll_for_completion(job_id, timeout: timeout)
  ...
  def fetch_job_results_after_completion(job_id)
    ...
    def poll_for_completion(job_id, timeout: 120, interval: 2)
      # Poll every 2 seconds for up to timeout seconds
      max_attempts = (timeout.to_f / interval).ceil
