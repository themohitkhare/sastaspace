...
Deploys to `https://almirah.sastaspace.com`.
...
- **k8s.yaml** — Deployment + Service + Ingress (`almirah.sastaspace.com`)
...
2. Apply project migrations (once):
   ...
   docker exec -i sastaspace-postgres psql -U postgres -d sastaspace \
     < projects/almirah/db/migrations/0001_init.sql
   ```
3. Run the web app:
   ```bash
   cd projects/almirah/web && npm install && npm run dev
   ```
