...
- For generated files (e.g. `Cargo.lock`, generated STDB bindings), resolve
  non-generated conflicts first, then regenerate:
  - Prefer resolving source files and handwritten logic before touching
    generated artifacts.
  - For `Cargo.lock`, prefer accepting one side and running `cargo build` to
    regenerate cleanly.
  - For STDB bindings (`crates/stdb-client/`), regenerate from the modules
    after the source-side conflict is resolved.
- For import/`use` conflicts where intent is unclear, accept both sides first:
  - Keep all candidate imports temporarily, finish the merge, then run
    `cargo clippy` to remove unused or incorrect imports safely.
