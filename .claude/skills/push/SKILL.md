...
2. Run local validation before pushing. Default sastaspace gauntlet:
   - `cargo fmt --check`
   - `cargo clippy --workspace -- -D warnings`
   - `cargo test --workspace`
   - When the change touches the TUI shell or app crates, also run
     `SPACETIME_PORT=3199 cargo test -p e2e` against a local fixture.
...
6. Write/update PR body:
   - If `.github/pull_request_template.md` exists, fill every section with
     concrete content for this change and replace all placeholder comments
     (`<!-- ... -->`).
   - If no template exists, use a brief `## Summary` + `## Test plan` section.
...
# Validation gauntlet (sastaspace defaults)
cargo fmt --check
cargo clippy --workspace -- -D warnings
cargo test --workspace
...
# Write/edit PR body. If a template exists, base content on it.
# Example workflow:
# 1) draft body content for this PR (using template if present)
