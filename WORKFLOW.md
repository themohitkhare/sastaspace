---
# Symphony policy file for sastaspace.
#
# Forward-looking: this file follows openai/symphony's WORKFLOW.md schema
# (https://github.com/openai/symphony/blob/main/SPEC.md) but the YAML
# frontmatter targets a Claude-based runner that does not exist yet. The
# workflow body (the prompt below) is what an agent would receive when this
# repo is wired into a Symphony-style daemon. Until that runner is built or
# adapted, treat this file as the canonical contract for how autonomous work
# on sastaspace tickets should behave.
#
# Background: https://openai.com/index/harness-engineering/

tracker:
  kind: linear
  project_slug: "sastaspace-201238489c0a"
  active_states:
    - Todo
    - In Progress
    - Merging
...
polling:
  interval_ms: 5000
workspace:
  root: ~/code/sastaspace-workspaces
hooks:
  after_create: |
    git clone --depth 1 git@github.com:themohitkhare/sastaspace .
    cargo build -p shell
agent:
  max_concurrent_agents: 5
  max_turns: 30
claude:
  command: claude --print --output-format stream-json --include-partial-messages --verbose
...
## Prerequisite: Linear MCP server is configured

The agent talks to Linear via the `plugin:linear:linear` MCP server. The relevant tools are namespaced `mcp__plugin_linear_linear__*` (e.g. `list_issues`, `save_issue`, `save_comment`). If the MCP server is not configured for this session, stop and ask the user to configure it.
...
- `linear`: interact with Linear via the MCP server.
- `commit`: produce clean, logical commits during implementation.
- `push`: keep remote branch current and publish updates.
- `pull`: keep branch updated with latest `origin/main` before handoff.
- `land`: when ticket reaches `Merging`, explicitly open and follow `.claude/skills/land/SKILL.md`.
...
1. Fetch the issue by explicit ticket ID using `mcp__plugin_linear_linear__get_issue`.
...
5. For `Todo` tickets, do startup sequencing in this exact order:
   ...
   - find/create `## Agent Workpad` bootstrap comment via `save_comment`.
...
1.  Find or create a single persistent scratchpad comment for the issue:
    - Search existing comments (via `list_comments`) for a marker header: `## Agent Workpad`.
...
5.  Ensure the workpad includes a compact environment stamp at the top as a code fence line:
    ...
    - Example: `devbox-01:/home/dev-user/code/sastaspace-workspaces/SAS-32@7bdde33bc`
    - Do not include metadata already inferable from Linear issue fields (`issue ID`, `status`, `branch`, `PR link`).
6.  Add explicit acceptance criteria and TODOs in checklist form in the same comment.
    - If changes are user-facing, include a TUI walkthrough acceptance criterion that describes the end-to-end user path to validate (the binary, key bindings, expected screen).
...
8.  Before implementing, capture a concrete reproduction signal and record it in the workpad `Notes` section (command/output, transcript snippet, or deterministic TUI behavior).
...
5.  Run validation/tests required for the scope.
    ...
    - Default validation gauntlet for sastaspace: `cargo fmt --check`, `cargo clippy --workspace -- -D warnings`, `cargo test --workspace`, and where applicable `SPACETIME_PORT=3199 cargo test -p e2e`.
