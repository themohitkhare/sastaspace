    ...
    async def _solve_loop(
        ...
        for gen in range(MAX_GENS_PER_SOLVE):
            ...
            if best_fitness >= 1.0 - 1e-5:
                ...
                await repo.update_match(match_id, {"status": MatchStatus.AI_WON.value})
                logger.info("SOLVED %s in %d generations!", match_id, generation_count + gen + 1)
                return
            ...
            # Trim to population size + tabu
            population = next_population[:population_size]
            tabu_hashes = trim_tabu(tabu_hashes)
