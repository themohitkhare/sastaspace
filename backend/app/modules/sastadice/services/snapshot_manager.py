...
class StateFrame:
    ...
    def __init__(
        ...
        invariant_status: list[str] = None
    ):
...
class SnapshotManager:
    ...
    def __init__(self, chaos_config: ChaosConfig | None = None):
        ...
        self._action_log: dict[str, list[dict]] = {}  # game_id -> actions
        logger.info(f"SnapshotManager initialized with history size: {self.HISTORY_SIZE}")
    
    def record_state(self, game: GameSession, action: dict | None = None) -> None:
        ...
        logger.debug(f"Recorded state for game {game.id[:8]}... (buffer size: {len(self._state_buffer[game.id])})")
