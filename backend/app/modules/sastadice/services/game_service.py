...
class GameOrchestrator:
    ...
    def _draw_event(self, game: GameSession) -> dict | None:
    ...
    async def _handle_sasta_event(self, game: GameSession, player: Player, tile: Tile) -> dict:
        ...
        for pid, amount in actions.get("cash_changes", {}).items():
            ...
            if p:
                if amount < 0:
                    await self._charge_player(
                        game, p, abs(amount), player if pid != player.id else None
                    )
                else:
                    p.cash += amount
                    await self.repository.update_player_cash(pid, p.cash)
    ...
    def _validate_trade_assets(
        self, game: GameSession, offer: TradeOffer, initiator: Player, target: Player
    ) -> str | None:
        return self.trade_manager.validate_trade_assets(game, offer, initiator, target)

    async def _execute_trade_transfer(
        self, game: GameSession, offer: TradeOffer, initiator: Player, target: Player
    ) -> None:
    ...
    async def _handle_end_turn(self, game: GameSession, player_id: str) -> ActionResult:
        """Handle end turn - delegates to turn coordinator."""
    ...
    async def _process_bankruptcy(
        self, game: GameSession, debtor: Player, creditor: Player | None = None
    ):
        ...
        for offer in list(game.active_trade_offers):
            ...
            if target:
                if random.random() < 0.3:
                    res = await self.perform_action(
                        game.id, target.id, ActionType.ACCEPT_TRADE, {"trade_id": offer.id}
                    )
                    turn_info["actions"].append(
                    ...
                    res = await self.perform_action(
                        game.id, target.id, ActionType.DECLINE_TRADE, {"trade_id": offer.id}
                    )
                    turn_info["actions"].append(
