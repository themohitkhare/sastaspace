...
if TYPE_CHECKING:
    ...
    from app.modules.sastadice.schemas import ChaosConfig, GameSession, Player
...
class MonkeyStrategy:
    ...
    def __init__(self, config: "ChaosConfig"):
        ...
        logger.info(
            f"MonkeyStrategy initialized with seed: {config.seed}, chaos_prob: {config.chaos_probability}"
        )
...
class ChaosRepository:
    ...
    def __init__(self, real_repo: "GameRepository", config: "ChaosConfig"):
        ...
        logger.info(
            f"ChaosRepository initialized with drop_db_writes={config.fault_injection.drop_db_writes}"
        )
    ...
    async def save_board(self, game_id: str, board: list[Any]) -> None:
