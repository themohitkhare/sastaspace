...
class InvariantViolationError(Exception):
    ...
    def __init__(self, violations: list[InvariantViolation]):
        ...
        super().__init__("Invariant violations detected:\n" + "\n".join(messages))
...
class InvariantChecker:
    ...
    def _check_asset_conservation(self, game: "GameSession") -> list[InvariantViolation]:
        ...
        for player in game.players:
            for prop_id in player.properties:
                found_tile: Tile | None = next((t for t in game.board if t.id == prop_id), None)
                if not found_tile:
                    violations.append(
                        InvariantViolation(
                ...
                elif found_tile.owner_id != player.id:
                    violations.append(
                        InvariantViolation(
                            type="OWNERSHIP_MISMATCH",
                            severity="CRITICAL",
                            message=f"Player {player.name} claims {found_tile.name} but tile owner is {found_tile.owner_id}",
                            game_id=game.id,
                            round_number=game.current_round,
                            turn_phase=game.turn_phase.value,
                        )
        ...
        for tile in property_tiles:
            if tile.owner_id:
                found_player: Player | None = next(
                    (p for p in game.players if p.id == tile.owner_id), None
                )
                if not found_player:
                    violations.append(
                        InvariantViolation(
                ...
                elif tile.id not in found_player.properties:
                    violations.append(
                        InvariantViolation(
                            type="MISSING_PROPERTY_REFERENCE",
                            severity="CRITICAL",
                            message=f"Tile {tile.name} owner is {found_player.name} but not in their properties list",
                            game_id=game.id,
                            round_number=game.current_round,
                            turn_phase=game.turn_phase.value,
                        )
    ...
    def check_financial_integrity(
        self, game: "GameSession", previous_system_cash: int, expected_delta: int
    ...
    def _check_position_bounds(self, game: "GameSession") -> list[InvariantViolation]:
        ...
        for player in game.players:
            ...
            if player.previous_position is not None and not (
                0 <= player.previous_position < board_size
            ):
                violations.append(
                    InvariantViolation(
    ...
    def _attempt_auto_corrections(
        self, game: "GameSession", violations: list[InvariantViolation]
    ) -> None:
        ...
        for v in violations:
            if v.type == "ORPHANED_PENDING_DECISION":
                ...
                if next_player:
                    ...
                    logger.warning(
                        f"Auto-corrected: Advanced to next player {next_player.name}: {v.message}"
                    )
    ...
    def _find_next_active_player(self, game: "GameSession") -> "Player | None":
        ...
        if game.current_turn_player_id:
            current_idx = next(
                (i for i, p in enumerate(game.players) if p.id == game.current_turn_player_id), 0
            )
