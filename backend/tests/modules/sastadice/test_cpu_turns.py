...
class TestCPUTurnLogic:
    ...
    def test_cpu_turn_completes_full_cycle(self, db_cursor):
        ...
        # Play CPU turn
        turn_log = service._play_cpu_turn(game, cpu_player)
    ...
    def test_cpu_handles_buy_decision(self, db_cursor):
        ...
        for _ in range(50):
            ...
            if game.turn_phase == TurnPhase.PRE_ROLL:
                service.perform_action(game.id, cpu_player.id, ActionType.ROLL_DICE, None)
                ...
                service.perform_action(game.id, cpu_player.id, ActionType.END_TURN, None)
            else:
                break

    def test_cpu_handles_insufficient_funds(self, db_cursor):
        ...
        service.repository.update_player_cash(cpu_player.id, 50)
    ...
    def test_cpu_handles_decision_without_pending_decision(self, db_cursor):
    ...
    def test_process_cpu_turns_handles_multiple_cpus(self, db_cursor):
        """Test process_cpu_turns handles multiple consecutive CPU turns."""
        service = GameService(db_cursor)
        game = service.create_game(cpu_count=3)
        game = service.start_game(game.id, force=True)

        # Process CPU turns
        result = service.process_cpu_turns(game.id)
    ...
    def test_cpu_turn_max_iterations_safety(self, db_cursor):
    ...
    def test_simulate_cpu_game_completes(self, db_cursor):
        """Test that simulate_cpu_game can complete a full game."""
        service = GameService(db_cursor)
        game = service.create_game(cpu_count=2)
        
        # Simulate game
        result = service.simulate_cpu_game(game.id, max_turns=50)
    ...
    def test_cpu_turn_handles_action_failures(self, db_cursor):
    ...
    def test_cpu_turn_player_not_found_handling(self, db_cursor):
        ...
        turn_log = service._play_cpu_turn(game, fake_player)
