...
import pytest
from app.modules.sastadice.schemas import GameSession, Player, Tile, TileType, GameSettings, ActionType
...
async def test_buy_release_action(db_database):
    ...
    game_doc = await orchestrator.repository.database.game_sessions.find_one({"_id": game.id})
    players_doc = await orchestrator.repository.database.players.find({"game_id": game.id}).to_list(None)
    ...
    result = await orchestrator.perform_action(
        game.id, player.id, ActionType.BUY_RELEASE, {}
    )
...
async def test_roll_for_doubles_action(db_database):
    ...
    result = await orchestrator.perform_action(
        game.id, player.id, ActionType.ROLL_FOR_DOUBLES, {}
    )
