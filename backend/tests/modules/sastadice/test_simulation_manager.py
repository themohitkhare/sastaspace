...
@pytest.mark.asyncio
async def test_attempt_cpu_trade_proposal_ignores_bankrupt_targets(simulation_manager, mock_dispatcher):
...
async def test_monkey_mode_trade_spam(mock_repository, mock_dispatcher):
    ...
    p1 = Player(id="p1", name="P1", cash=1000)
    p2 = Player(id="p2", name="P2", cash=1000)
    target_prop = Tile(id="t1", name="Target Prop", type=TileType.PROPERTY, owner_id="p2", price=200)
    
    game = GameSession(id="g1", status=GameStatus.ACTIVE, players=[p1, p2], current_turn_player_id="p1", board=[target_prop])
    manager._get_game.return_value = game
    ...
    calls = [c for c in mock_dispatcher.dispatch.mock_calls if c.args[2] == ActionType.PROPOSE_TRADE]
    assert len(calls) > 0
