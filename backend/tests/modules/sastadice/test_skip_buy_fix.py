    ...
    if chance_tile:
        ...
        if game.turn_phase == TurnPhase.DECISION and game.pending_decision:
            pass_result = await service.perform_action(
                game.id, player.id, ActionType.PASS_PROPERTY, {}
            )
...
async def test_skip_buy_clears_pending_decision(db_database):
    ...
    property_tile = next(
        (t for t in game.board if t.type == TileType.PROPERTY and t.owner_id is None), None
    )
